...
export function amazonUrl(value: string): boolean {
  try {
    ...
    return u.protocol === "https:" && !u.username && !u.password && /^(?:(?:www\.|smile\.)?amazon\.(?:com|co\.uk|ca|de|fr|it|es|co\.jp|in|com\.au|com\.br|com\.mx|nl|se|pl|sg|ae|sa|com\.tr|eg|co\.za|ie)|amzn\.to|a\.co|link\.amazon)$/.test(u.hostname);
  } catch { return false; }
}
...
export function validateAffiliateSection(value: unknown): AffiliateSection {
  ...
  const products = data.products.map((raw: unknown, index: number) => {
    ...
    const label = `Product ${index + 1}${product.name ? ` (${product.name})` : ""}`;
    if (!product.id) throw new Error(`${label}: missing product ID. Remove this product and add it again.`);
    if (!product.name) throw new Error(`${label}: enter a product name.`);
    if (!product.url) throw new Error(`${label}: enter the Amazon affiliate link.`);
    if (!amazonUrl(product.url)) throw new Error(`${label}: the Amazon affiliate link is not supported. Use a full https:// Amazon product link or an https://amzn.to/ or https://link.amazon/ short link.`);
    if (!product.image) throw new Error(`${label}: add a product image URL or upload an image.`);
    if (!safeImageUrl(product.image)) throw new Error(`${label}: the product image URL is invalid. Use an https:// image URL or the /blog-images/ path returned by an upload.`);
